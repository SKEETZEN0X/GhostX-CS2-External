...
void render::run( )
{
	...
	while ( true )
	{
		if ( g::should_exit.load( std::memory_order_relaxed ) )
		{
		...
		ImGui::NewFrame( );
		{
			...
			{
				welcome_sent = true;
				notify->add_notify( "GhostX ready - welcome back", notify_type::success );
			}
			notify->setup_notify( );
		}
		if ( g::menu.is_open( ) )
...
void render::update_input_window( )
{
	...
	if ( g::menu.is_open( ) )
	{
		const auto new_style = style & ~( WS_EX_TRANSPARENT | WS_EX_NOACTIVATE );
		if ( new_style != style )
		{
			::SetWindowLongW( this->m_hwnd, GWL_EXSTYLE, new_style );
