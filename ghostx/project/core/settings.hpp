...
struct combat {
  struct aimbot {
    ...
    bool head_only{
        true}; // Kept for backwards compatibility or fallback if needed
    bool target_head{true};
    bool target_chest{true};
    bool target_stomach{true};
    bool target_legs{false};
    ...
    int target_switch_delay_ms{0};  // delay before switching to new target after current target dies (0 = instant)
  };
...
struct esp {
  struct player {
    ...
    struct info_flags {
      ...
      bool enabled{true};
      std::uint8_t flags{flag::money | flag::armor | flag::kit | flag::scoped |
                         flag::defusing | flag::flashed | flag::ping};
    ...
    struct head_dot {
      bool enabled{true};
      zdraw::rgba color{220, 100, 100, 255};
      float radius{3.5f};
      bool outline{true};
    } m_head_dot{};
  } m_player{};
