...
void player::on_render() {
  ...
  const auto global_vars =
      g::memory.read<std::uintptr_t>(g::offsets.global_vars);
  if (!global_vars) {
    return;
  }
  ...
  for (const auto &player : systems::g_collector.players()) {
    ...
    if (cfg.m_snaplines.enabled) {
      this->add_snaplines(bounds, player, cfg.m_snaplines);
    }

    if (cfg.m_head_dot.enabled) {
      this->add_head_dot(bones, player, cfg.m_head_dot);
    }
  }
}

void player::add_box(const systems::bounds::data &bounds,
                     const settings::esp::player::box &cfg, bool is_visible) {
  ...
  if (cfg.fill) {
    ...
    const auto edge_color =
        zdraw::rgba(static_cast<std::uint8_t>(edge_r * 255),
                    static_cast<std::uint8_t>(edge_g * 255),
                    static_cast<std::uint8_t>(edge_b * 255),
                    static_cast<std::uint8_t>(255 * edge_alpha));
    const auto center_color =
        zdraw::rgba(static_cast<std::uint8_t>(edge_r * 255 * center_brightness),
                    static_cast<std::uint8_t>(edge_g * 255 * center_brightness),
                    static_cast<std::uint8_t>(edge_b * 255 * center_brightness),
                    static_cast<std::uint8_t>(255 * center_alpha));

    const auto mid_y = y + h * 0.5f;
    zdraw::rect_filled_multi_color(x + 1, y + 1, w - 2, mid_y - y - 1,
                                   edge_color, edge_color, center_color,
                                   center_color);
    zdraw::rect_filled_multi_color(x + 1, mid_y, w - 2, y + h - mid_y - 1,
                                   center_color, center_color, edge_color,
                                   edge_color);
  }
  ...
  if (cfg.style == settings::esp::player::box::style0::full) {
    ...
    if (cfg.outline) {
      zdraw::rect_cornered(x - 1, y - 1, w + 2, h + 2,
                           zdraw::rgba(0, 0, 0, 180), corner + 1, 1.0f);
...
void player::add_skeleton(const systems::bones::data &bones,
                          const settings::esp::player::skeleton &cfg,
                          bool is_visible) {
  const auto &color = is_visible ? cfg.visible_color : cfg.occluded_color;
  ...
  for (const auto &[from, to] : connections) {
    ...
    if (!systems::g_view.projection_valid(from_screen) ||
        !systems::g_view.projection_valid(to_screen)) {
      continue;
    }

    zdraw::line(from_screen.x, from_screen.y, to_screen.x, to_screen.y, color,
                cfg.thickness);
  }
}

void player::add_hitboxes(const systems::bones::data &bones,
                          const systems::collector::player &player,
                          const settings::esp::player::hitboxes &cfg,
                          float current_time) {
  const auto &color =
      player.is_visible ? cfg.visible_color : cfg.occluded_color;
  ...
  const auto flash_t = std::clamp(
      1.0f - (current_time - anim.last_damage_time) * 2.5f, 0.0f, 1.0f);
  ...
  for (const auto &hb : player.hitboxes) {
    ...
    const auto center_world = bone.position + math::helpers::rotate_by_quat(
                                                  bone.rotation, center_local);

    const auto half_extent = (hb.maxs - hb.mins) * 0.5f;
    const auto longest =
        std::max({std::abs(half_extent.x), std::abs(half_extent.y),
                  std::abs(half_extent.z)});

    math::vector3 axis_local{};
    if (std::abs(half_extent.x) >= std::abs(half_extent.y) &&
        std::abs(half_extent.x) >= std::abs(half_extent.z)) {
    ...
    const auto axis_world =
        math::helpers::rotate_by_quat(bone.rotation, axis_local);
    ...
    if (!systems::g_view.projection_valid(sa) ||
        !systems::g_view.projection_valid(sb)) {
      continue;
    }
    ...
    if (!systems::g_view.projection_valid(p_left) ||
        !systems::g_view.projection_valid(p_right)) {
      continue;
    }

    const auto screen_radius =
        std::sqrt((p_left.x - p_right.x) * (p_left.x - p_right.x) +
    ...
    pills.push_back(
        poly2d::make_pill({sa.x, sa.y}, {sb.x, sb.y}, screen_radius, 4));
  }
  ...
  for (const auto &outline : merged.outlines) {
    ...
    if (cfg.fill) {
      ...
      if (hp < 1.0f || flash_t > 0.0f) {
        ...
        const auto flash_alpha = static_cast<std::uint8_t>(
            base_alpha + (200 - base_alpha) * flash_t);
        ...
        auto clip_triangle_above = [&](float x0, float y0, float x1, float y1,
                                       float x2, float y2) {
          const auto a0 = y0 >= fill_line;
          const auto a1 = y1 >= fill_line;
          const auto a2 = y2 >= fill_line;
          const auto count = static_cast<int>(a0) + static_cast<int>(a1) +
                             static_cast<int>(a2);
          ...
          if (count == 3) {
            clipped.push_back(x0);
            clipped.push_back(y0);
            clipped.push_back(x1);
            clipped.push_back(y1);
            clipped.push_back(x2);
            clipped.push_back(y2);
            return;
          }

          auto lerp_x = [](float ax, float ay, float bx, float by, float cy) {
          ...
          if (count == 1) {
            ...
            if (a0) {
              tx = x0;
              ty = y0;
              ix0 = lerp_x(x0, y0, x1, y1, fill_line);
              ix1 = lerp_x(x0, y0, x2, y2, fill_line);
            } else if (a1) {
              tx = x1;
              ty = y1;
              ix0 = lerp_x(x1, y1, x0, y0, fill_line);
              ix1 = lerp_x(x1, y1, x2, y2, fill_line);
            } else {
              tx = x2;
              ty = y2;
              ix0 = lerp_x(x2, y2, x0, y0, fill_line);
              ix1 = lerp_x(x2, y2, x1, y1, fill_line);
            }

            clipped.push_back(tx);
            clipped.push_back(ty);
            clipped.push_back(ix0);
            clipped.push_back(fill_line);
            clipped.push_back(ix1);
            clipped.push_back(fill_line);
          } else {
            ...
            clipped.push_back(kx0);
            clipped.push_back(ky0);
            clipped.push_back(kx1);
            clipped.push_back(ky1);
            clipped.push_back(ix0);
            clipped.push_back(fill_line);
        ...
        for (std::size_t i = 0; i + 5 < tris.size(); i += 6) {
          clip_triangle_above(tris[i], tris[i + 1], tris[i + 2], tris[i + 3],
                              tris[i + 4], tris[i + 5]);
        }

        for (std::size_t i = 0; i + 5 < clipped.size(); i += 6) {
          zdraw::triangle_filled(clipped[i], clipped[i + 1], clipped[i + 2],
                                 clipped[i + 3], clipped[i + 4], clipped[i + 5],
                                 red);
        }
      }
    }
    ...
    if (cfg.outline) {
      ...
      zdraw::polyline(std::span<const float>(flat.data(), flat.size()), color,
                      true);
    }
  }
}

void player::add_health_bar(const systems::bounds::data &bounds,
                            const systems::collector::player &player,
                            const settings::esp::player::health_bar &cfg,
                            draw_offsets &offsets) {
  auto &anim = this->m_animations[player.controller];
  ...
  const auto vertical =
      cfg.position == settings::esp::player::health_bar::position::left;
  ...
  const auto x = [&]() {
    if (cfg.position == settings::esp::player::health_bar::position::left) {
      return std::floorf(bounds.min.x - bar_size - padding - offsets.left -
                         outline_size);
    }
  ...
  const auto y = [&]() {
    switch (cfg.position) {
    case settings::esp::player::health_bar::position::left:
      return std::floorf(bounds.min.y);
    case settings::esp::player::health_bar::position::top:
      return std::floorf(bounds.min.y - bar_size - padding - offsets.top -
                         outline_size);
    case settings::esp::player::health_bar::position::bottom:
      return std::floorf(bounds.max.y + padding + offsets.bottom +
                         outline_size);
    }
    return 0.0f;
  }();
  ...
  if (filled > 0) {
    if (cfg.gradient) {
      if (vertical) {
        zdraw::rect_filled_multi_color(x, y + bar_h - filled, bar_w, filled,
                                       cfg.full_color, cfg.full_color,
                                       cfg.low_color, cfg.low_color);
      } else {
        zdraw::rect_filled_multi_color(x, y, filled, bar_h, cfg.low_color,
                                       cfg.full_color, cfg.full_color,
                                       cfg.low_color);
      }
    } else {
      if (vertical) {
        zdraw::rect_filled(x, y + bar_h - filled, bar_w, filled,
                           cfg.full_color);
      } else {
  ...
  if (cfg.show_value && clamped_health < 100) {
    ...
    const auto text_y = vertical
                            ? std::floorf(y + bar_h - filled - text_h - 2.0f)
                            : std::floorf(y - text_h - 2.0f);
...
void player::add_ammo_bar(const systems::bounds::data &bounds,
                          const systems::collector::player &player,
                          const settings::esp::player::ammo_bar &cfg,
                          draw_offsets &offsets) {
  auto &anim = this->m_animations[player.controller];
  ...
  const auto clamped_ammo =
      std::clamp(player.weapon.ammo, 0, player.weapon.max_ammo);
  const auto target_fraction =
      static_cast<float>(clamped_ammo) / player.weapon.max_ammo;
  ...
  const auto outline_size = cfg.outline ? 1.0f : 0.0f;
  const auto vertical =
      cfg.position == settings::esp::player::ammo_bar::position::left;
  ...
  const auto y = [&]() {
    switch (cfg.position) {
    case settings::esp::player::ammo_bar::position::left:
      return std::floorf(bounds.min.y);
    case settings::esp::player::ammo_bar::position::top:
      return std::floorf(bounds.min.y - bar_size - padding - offsets.top -
                         outline_size);
    case settings::esp::player::ammo_bar::position::bottom:
      return std::floorf(bounds.max.y + padding + offsets.bottom +
                         outline_size);
    }
    return 0.0f;
  ...
  if (cfg.show_value) {
    ...
    const auto text =
        std::format("{}/{}", clamped_ammo, player.weapon.max_ammo);
    const auto [text_w, text_h] = zdraw::measure_text(text);
    const auto text_x = std::floorf(x + (bar_w * 0.5f) - (text_w * 0.5f));
    const auto text_y = vertical
                            ? std::floorf(y + bar_h - filled - text_h - 2.0f)
                            : std::floorf(y + bar_h + 2.0f);
...
void player::add_name(const systems::bounds::data &bounds,
                      const systems::collector::player &player,
                      const settings::esp::player::name &cfg,
                      draw_offsets &offsets) {
  zdraw::push_font(g::render.fonts().pretzel_12);

  const auto [text_w, text_h] = zdraw::measure_text(player.display_name);
  const auto text_x =
      std::floorf(bounds.min.x + (bounds.width() * 0.5f) - (text_w * 0.5f));
  const auto text_y = std::floorf(bounds.min.y - text_h - 2.0f - offsets.top);

  zdraw::text<zdraw::tstyles::outlined>(text_x, text_y, player.display_name,
                                        cfg.color);
  zdraw::pop_font();
...
void player::add_weapon(const systems::bounds::data &bounds,
                        const systems::collector::player &player,
                        const settings::esp::player::weapon &cfg,
                        draw_offsets &offsets) {
  zdraw::push_font(g::render.fonts().mochi_12);

  auto total_height{0.0f};

  if (cfg.display == settings::esp::player::weapon::display_type::icon ||
      cfg.display ==
          settings::esp::player::weapon::display_type::text_and_icon) {
    ...
    const auto icon_x =
        std::floorf(bounds.min.x + (bounds.width() * 0.5f) - (icon_w * 0.5f));
    const auto icon_y =
  ...
  if (cfg.display == settings::esp::player::weapon::display_type::text ||
      cfg.display ==
          settings::esp::player::weapon::display_type::text_and_icon) {
    ...
    zdraw::text<zdraw::tstyles::outlined>(text_x, text_y, player.weapon.name,
                                          cfg.text_color);
...
void player::add_flags(const systems::bounds::data &bounds,
                       const systems::collector::player &player,
                       const settings::esp::player::info_flags &cfg,
                       draw_offsets &offsets) {
  zdraw::push_font(g::render.fonts().pixel7_10);
  ...
  const auto draw_flag = [&](const std::string &text,
  ...
  if (cfg.has(settings::esp::player::info_flags::flag::armor) &&
      player.armor > 0) {
    draw_flag(player.has_helmet ? "hk" : "k", cfg.armor_color);
  }

  if (cfg.has(settings::esp::player::info_flags::flag::kit) &&
      player.has_defuser) {
    draw_flag("kit", cfg.kit_color);
  }

  if (cfg.has(settings::esp::player::info_flags::flag::scoped) &&
      player.is_scoped) {
    draw_flag("zoom", cfg.scoped_color);
  }

  if (cfg.has(settings::esp::player::info_flags::flag::defusing) &&
      player.is_defusing) {
    draw_flag("defusing", cfg.defusing_color);
  }

  if (cfg.has(settings::esp::player::info_flags::flag::flashed) &&
      player.is_flashed) {
  ...
  if (cfg.has(settings::esp::player::info_flags::flag::ping)) {
    const auto ping_color = [&]() -> zdraw::rgba {
      if (player.ping <= 20)
        return {98, 217, 109, 255};
      if (player.ping <= 50)
        return {230, 206, 137, 255};
      if (player.ping <= 70)
        return {230, 156, 110, 255};
  ...
  if (cfg.has(settings::esp::player::info_flags::flag::distance)) {
    const auto distance =
        systems::g_view.origin().distance(player.origin) * 0.01905f;
...
void player::add_snaplines(const systems::bounds::data &bounds,
                           const systems::collector::player &player,
                           const settings::esp::player::snaplines &cfg) {
  const auto [w, h] = zdraw::get_display_size();
  const auto start_x = w * 0.5f;
  const auto start_y = (float)h;

  const auto end_x = bounds.min.x + bounds.width() * 0.5f;
  const auto end_y = bounds.max.y;

  zdraw::line(start_x, start_y, end_x, end_y, cfg.color, cfg.thickness);
}

void player::add_head_dot(const systems::bones::data &bones,
                          const systems::collector::player &player,
                          const settings::esp::player::head_dot &cfg) {
  auto head_pos = bones.get_position(cstypes::head);
  const auto trace_result =
      systems::g_bvh.trace_ray(systems::g_view.origin(), head_pos);
  const auto is_visible = !trace_result.hit || trace_result.fraction > 0.97f;

  if (!is_visible) {
    return; // Only draw head dot if visible
  }
  ...
  if (cfg.outline) {
    zdraw::circle_filled(head_screen.x, head_screen.y, cfg.radius + 1.0f,
                         zdraw::rgba(0, 0, 0, 200), 16);
  }
  zdraw::circle_filled(head_screen.x, head_screen.y, cfg.radius, cfg.color, 16);
}
