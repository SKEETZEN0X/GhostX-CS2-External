...
public:
  ...
  template <typename T> [[nodiscard]] T get(std::uintptr_t cvar_ptr);
};

class schemas {
public:
  [[nodiscard]] std::int32_t lookup(const char *class_name,
...
class entities {
public:
  enum class type : std::uint8_t { unknown, player, item, projectile };
...
private:
  ...
  [[nodiscard]] std::uintptr_t get_by_index(std::uintptr_t entity_list,
...
public:
  ...
  [[nodiscard]] bool projection_valid(const math::vector2 &screen_pos) {
    return screen_pos.x != this->k_invalid && screen_pos.y != this->k_invalid;
  }
...
public:
  ...
  struct set {
    ...
    [[nodiscard]] const entry *end() const {
      return this->entries.data() + this->count;
    }
  };
...
private:
  static constexpr int k_bone_map[]{6,  -1, 0,  1,  2,  3, 4, 22, 25, 23,
                                    26, 24, 27, 10, 15, 9, 8, 14, 13};
};
...
private:
  ...
  [[nodiscard]] static projectile_subtype
  classify_projectile(std::uint32_t schema_hash);
...
public:
  ...
  [[nodiscard]] trace_result trace_ray(const math::vector3 &start,
                                       const math::vector3 &end,
                                       std::int32_t exclude_tri = -1) const;
  [[nodiscard]] std::vector<hit_entry>
  trace_ray_all(const math::vector3 &start, const math::vector3 &end) const;
  [[nodiscard]] std::vector<penetration_segment>
  build_segments(const std::vector<hit_entry> &hits, float ray_length) const;
...
private:
  ...
  void rebuild_accel();
  std::int32_t build_recursive(std::int32_t start, std::int32_t end,
                               std::int32_t depth);
