...
template <typename F>
static scan_result scan_chunked(const memory &mem, std::uintptr_t base,
...
static std::vector<section_info> get_sections(const memory &mem,
  ...
  const auto first = module_base + dos.e_lfanew +
  ...
  for (std::uint16_t i = 0; i < nt.FileHeader.NumberOfSections; ++i) {
    const auto hdr = mem.read<IMAGE_SECTION_HEADER>(
        first + i * sizeof(IMAGE_SECTION_HEADER));
...
bool memory::initialize(std::wstring_view process_name) {
  ...
  this->m_handle = ::OpenProcess(PROCESS_VM_READ | PROCESS_QUERY_INFORMATION,
                                 FALSE, this->m_id);
  ...
  const auto mod_snap = ::CreateToolhelp32Snapshot(
      TH32CS_SNAPMODULE | TH32CS_SNAPMODULE32, this->m_id);
...
bool memory::read(std::uintptr_t address, void *buffer,
  ...
  std::size_t bytes{0};
  return ::ReadProcessMemory(this->m_handle,
...
std::uintptr_t memory::get_module(std::string_view name) const {
  ...
  const auto snap = ::CreateToolhelp32Snapshot(
      TH32CS_SNAPMODULE | TH32CS_SNAPMODULE32, this->m_id);
  if (snap == INVALID_HANDLE_VALUE) {
    return 0;
  }
  ...
  const auto len = ::MultiByteToWideChar(
      CP_UTF8, 0, name.data(), static_cast<int>(name.size()), wide, MAX_PATH);
...
std::string memory::read_string(std::uintptr_t address,
...
std::uintptr_t memory::find_pattern(std::uintptr_t module_base,
  ...
  const auto result = detail::scan_chunked(
      *this, module_base, module_size, parsed.size(),
...
std::uintptr_t
memory::find_qword_in_sections(std::uintptr_t module_base, std::uintptr_t value,
                               std::uint32_t section_filter) const {
...
std::uintptr_t memory::find_vtable(std::uintptr_t module_base,
  ...
  for (const auto &sec : detail::get_sections(*this, module_base)) {
    ...
    if ((sec.m_characteristics & required) != required ||
        sec.m_size <= descriptor_name.size()) {
      continue;
    }

    const auto result = detail::scan_chunked(
        *this, sec.m_base, sec.m_size, descriptor_name.size(),
        [&](const std::uint8_t *data, std::size_t size) -> std::size_t {
          for (std::size_t i = 0; i + descriptor_name.size() < size; ++i) {
            if (std::memcmp(data + i, descriptor_name.data(),
  ...
  const auto descriptor_rva =
      static_cast<std::uint32_t>(type_descriptor - module_base);
  std::uintptr_t col_address{0};

  for (const auto &sec : detail::get_sections(*this, module_base)) {
    if (std::string_view{sec.m_name}.find(".rdata") == std::string_view::npos ||
        sec.m_size < 0x30) {
      continue;
    }

    const auto result = detail::scan_chunked(
        *this, sec.m_base, sec.m_size, 0x30,
        [&](const std::uint8_t *data, std::size_t size) -> std::size_t {
          for (std::size_t i = 0; i + 0x30 <= size; i += 8) {
            if (reinterpret_cast<const std::uint32_t *>(data + i)[3] ==
                descriptor_rva) {
              return i;
            }
          }
  ...
  const auto col_ref = this->find_qword_in_sections(module_base, col_address,
                                                    IMAGE_SCN_MEM_READ);
  return col_ref ? col_ref + 8 : 0;
}

std::uintptr_t memory::find_vtable_instance(std::uintptr_t module_base,
                                            std::string_view class_name) const {
  ...
  return this->find_qword_in_sections(module_base, vtable,
