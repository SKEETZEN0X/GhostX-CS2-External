...
void c_gui::draw_decoration()
{
    ...
    const ImVec2 window_size = SCALE(var->window.window_size);
    ...
    draw->rect_filled(drawlist, pos, pos + window_size, draw->get_clr(clr->main.dark_1), SCALE(elements->rounding * 3));
    draw->rect_filled(drawlist, pos, pos + titlebar_size, draw->get_clr(clr->main.dark_2), SCALE(elements->rounding * 3), ImDrawFlags_RoundCornersTop);
    ...
    const char* label = "Given";
    const char* label_cc = "chy";
    const ImVec2 label_size = gui->text_size(var->font.medium15, label);

    draw->text(drawlist, var->font.medium15, var->font.medium15->FontSize, pos + ImVec2(SCALE(15.f), titlebar_size.y / 2 - label_size.y / 2), draw->get_clr(clr->text.active), label);
    draw->text(drawlist, var->font.medium15, var->font.medium15->FontSize, pos + ImVec2(SCALE(15.f) + label_size.x, titlebar_size.y / 2 - label_size.y / 2), draw->get_clr(clr->accent.main), label_cc);
}
