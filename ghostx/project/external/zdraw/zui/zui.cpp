...
namespace
{
	...
	{
	public:
		combo_overlay( widget_id id, const zui::rect& anchor, float width, const std::vector<std::string>& items, int initial_selection, std::function<void( )> on_change = nullptr ) : overlay{ id, anchor }, m_width{ width }, m_items{ items }, m_selected_index{ initial_selection }, m_on_change{ std::move( on_change ) }
		{
			...
			{
				this->m_selected_anims[ m_selected_index ] = 1.0f;
			}
		}

		[[nodiscard]] int get_selected_index( ) const noexcept { return m_selected_index; }
		...
		void render( const zui::style& style, const input_state& input ) override
		{
			...
			for ( int i = 0; i < static_cast< int >( this->m_items.size( ) ); ++i )
			{
				...
				const auto is_selected = ( i == this->m_selected_index );
	...
	public:
		...
		std::unordered_map<widget_id, float> m_scroll_targets{};
		std::unordered_map<widget_id, float> m_scroll_velocities{};
...
namespace zui {
	...
	namespace
	{
		...
		void process_scroll_wheel( const rect& bounds, float content_height, float visible_start_y, float& scroll_y, widget_id scroll_id = 0 )
		{
			...
			auto& target = ctx( ).m_scroll_targets[ scroll_id ];
			auto& velocity = ctx( ).m_scroll_velocities[ scroll_id ];
			...
			if ( input.hovered( bounds ) && !ctx( ).overlay_blocking_input( ) )
			{
				...
				{
					target -= scroll_delta * 55.0f;
				}
			}

			target = std::clamp( target, 0.0f, max_scroll );

			const auto diff = target - scroll_y;
			velocity = diff * 18.0f * dt;
			scroll_y += velocity;
	...
	bool begin_window( std::string_view title, float& x, float& y, float& w, float& h, bool resizable, float min_w, float min_h )
	{
		...
		if ( scroll_states.find( id ) == scroll_states.end( ) )
		{
			...
			ctx( ).m_scroll_targets[ id ] = 0.0f;
			ctx( ).m_scroll_velocities[ id ] = 0.0f;
		}
	...
	void end_window( )
	{
		...
		if ( win && win->scroll_id != invalid_id )
		{
			...
			process_scroll_wheel( win->bounds, win->content_height, win->visible_start_y, scroll_y, win->scroll_id );
	...
	bool keybind( std::string_view label, int& key )
	{
		...
		static int s_keybind_grace_frames{ 0 };
		if ( pressed )
		{
			ctx( ).m_active_keybind_id = id;
			s_keybind_grace_frames = 2;
			for ( int vk = 0x01; vk <= 0xFE; ++vk )
				GetAsyncKeyState( vk );
		}
		if ( s_keybind_grace_frames > 0 )
			--s_keybind_grace_frames;
		...
		if ( is_waiting && s_keybind_grace_frames == 0 )
		{
	...
	bool combo( std::string_view label, int& current_item, const char* const items[ ], int items_count, float width )
	{
		...
		if ( auto* popup = dynamic_cast< combo_overlay* >( overlays.find( id ) ) )
		{
			...
			if ( changed )
				current_item = popup->get_selected_index( );
		}
