...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    bool hovered, held;
    bool pressed = ButtonBehavior(bb, id, &hovered, &held);
    const ImGuiID popup_id = ImHashStr("##StyledComboPopup", 0, id);
    ...
    // --- Couleurs stylis�es ---
    ImU32 frame_col = GetColorU32(hovered ? ImVec4(0.18f, 0.18f, 0.18f, 1.0f)
        : ImVec4(0.12f, 0.12f, 0.12f, 1.0f));
    ImU32 border_col = GetColorU32(ImVec4(1.0f, 0.84f, 0.0f, hovered ? 1.0f : 0.6f)); // dor�
    ImU32 arrow_col = GetColorU32(ImVec4(1.0f, 1.0f, 1.0f, 0.9f));

    // --- Fond avec arrondi ---
    window->DrawList->AddRectFilled(bb.Min, bb.Max, frame_col, style.FrameRounding);
    window->DrawList->AddRect(bb.Min, bb.Max, border_col, style.FrameRounding, 0, 2.0f);

    // --- Texte preview ---
    if (preview_value != NULL && !(flags & ImGuiComboFlags_NoPreview))
    {
        ImVec2 text_pos = bb.Min + style.FramePadding;
        RenderTextClipped(text_pos, bb.Max - ImVec2(arrow_size, 0), preview_value, NULL, NULL);
    }

    // --- Fl�che custom ---
    if (!(flags & ImGuiComboFlags_NoArrowButton))
    {
        ImVec2 center = ImVec2(bb.Max.x - arrow_size * 0.5f, (bb.Min.y + bb.Max.y) * 0.5f);
        float s = 5.0f;
        window->DrawList->AddTriangleFilled(
            ImVec2(center.x - s, center.y - 2),
            ImVec2(center.x + s, center.y - 2),
...
#include <givenchy/settings/functions.h>
...
{
    struct anim_state {
        float cursor_pos, cursor_anim = 0.f;
    };
    ...
    if (is_multiline) // Open group before calling GetID() because groups tracks id created within their scope (including the scrollbar)
        BeginGroup();
    const ImGuiID id = window->GetID(label);
    anim_state* astate = gui->anim_container(&astate, id);
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    ...
    if (render_cursor || render_selection)
    {
            ...
            for (const char* p = text_selected_begin; p < text_selected_end; )
            {
                ...
                else
                {
                    ...
                    if (rect.Overlaps(clip_rect))
                        draw_window->DrawList->AddRectFilled(rect.Min, rect.Max, bg_color, SCALE(elements->rounding), ImDrawFlags_RoundCornersAll);
        ...
        if (render_cursor)
        {
            ...
            gui->easing(astate->cursor_pos, cursor_screen_pos.x - draw_pos.x, var->anim.speed * 2, dynamic_easing);
            ImRect cursor_screen_rect(draw_pos.x + astate->cursor_pos, cursor_screen_pos.y - g.FontSize + 0.5f, draw_pos.x + astate->cursor_pos + 1.0f, cursor_screen_pos.y - 1.5f);
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    ImVec2 size(
        size_arg.x != 0.0f ? size_arg.x : label_size.x + style.FramePadding.x * 2.0f,
    ...
    // --- Palette custom ---
    ImVec4 baseCol = ImVec4(0.10f, 0.10f, 0.10f, 0.9f);  // fond normal
    ImVec4 hoverCol = ImVec4(0.20f, 0.20f, 0.20f, 1.0f);  // hover fonc�
    ImVec4 selectCol = ImVec4(1.0f, 0.84f, 0.0f, 0.25f);   // dor� transparent
    ImVec4 borderCol = ImVec4(1.0f, 0.84f, 0.0f, 0.9f);    // bordure dor�e
    ImVec4 textCol = ImVec4(0.95f, 0.95f, 0.95f, 1.0f);  // texte blanc
    ImVec4 textColSel = ImVec4(1.0f, 0.9f, 0.5f, 1.0f);     // texte dor� clair si s�lectionn�

    ImU32 col_bg;
    if (selected)
        col_bg = GetColorU32(selectCol);
    else if (hovered)
        col_bg = GetColorU32(hoverCol);
    else
        col_bg = GetColorU32(baseCol);

    // --- Fond arrondi ---
    window->DrawList->AddRectFilled(bb.Min, bb.Max, col_bg, style.FrameRounding);

    // Bordure fine dor�e si s�lectionn�
    if (selected)
        window->DrawList->AddRect(bb.Min, bb.Max, GetColorU32(borderCol), style.FrameRounding, 0, 1.5f);

    // --- Texte centr� verticalement ---
    ImVec2 text_size = CalcTextSize(label, NULL, true);
    ImU32 col_text = GetColorU32(selected ? textColSel : textCol);
    ...
    window->DrawList->AddText(final_pos, col_text, label);
