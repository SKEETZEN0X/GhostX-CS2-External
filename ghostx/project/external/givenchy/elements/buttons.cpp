...
bool c_widgets::keybind_button(std::string_view label, int* key) {
    ...
    ImVec2 label_size = gui->text_size(var->font.medium10, state->buf_display);
    ...
    bool wnd_hovered = (g.HoveredWindow == window) || (g.HoveredWindow && g.HoveredWindow->ParentWindow == window);
    bool hovered = total_bb.Contains(GetMousePos()) && wnd_hovered;
    bool add_hovered = plus_bb.Contains(GetMousePos()) && wnd_hovered;
    ...
    if (*key != 0 && g.ActiveId != id) {
        strcpy_s(state->buf_display, active_key.c_str());
    }
    else if (g.ActiveId == id) {
        strcpy_s(state->buf_display, "...");
    }
    ...
    else if (!just_activated)
    {
        if (IsKeyPressed(ImGuiKey_Escape)) {
            *key = 0;
            ...
            if (!value_changed && g.IO.MouseClicked[0]) {
                k = 0x01;
                value_changed = true;
                ImGui::ClearActiveID();
            }
    ...
    draw->text_clipped(drawlist, var->font.medium10, label_bb.Min, label_bb.Max, draw->get_clr(state->text_color), state->buf_display);
    ...
    if (add_hovered && IsMouseClicked(0)) {
        strcpy_s(state->buf_display, "None");
    }
