...
bool c_widgets::direct_tab_button(std::string_view label, int tab_index)
{
    struct anim_state {
    ...
    const bool is_active = (var->tab.active == tab_index);

    gui->easing(state->text_color, is_active ? clr->text.active.Value : (hovered || g.LastActiveId == id) ? clr->text.hover.Value : clr->text.inactive.Value, var->anim.speed, dynamic_easing);

    // Accent underline for active tab
    if ( is_active || state->border_color.w > 0.01f )
    {
        gui->easing(state->border_color, is_active ? clr->accent.main.Value : clr->accent.invisble.Value, var->anim.speed, dynamic_easing);
        const float line_h = SCALE(2.5f);
        drawlist->AddRectFilled(
            ImVec2( bb.Min.x + padding.x * 0.5f, bb.Max.y - line_h ),
            ImVec2( bb.Max.x - padding.x * 0.5f, bb.Max.y ),
            draw->get_clr(state->border_color), SCALE(1.f) );
    }
    else
    {
        gui->easing(state->border_color, clr->accent.invisble.Value, var->anim.speed, dynamic_easing);
    }

    draw->text_clipped(drawlist, var->font.medium12, label_bb.Min, label_bb.Max, draw->get_clr(state->text_color), label.data());

    if (pressed) {
        var->tab.active = tab_index;
