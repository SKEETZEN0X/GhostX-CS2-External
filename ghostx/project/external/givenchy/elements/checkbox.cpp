...
bool c_widgets::checkbox_ex(std::string_view label, std::string_view description, bool* value, bool is_keybind, bool is_color, float color[4], bool alpha, bool is_keybind_window) {
    ...
    gui->easing(state->button_background, *value ? clr->accent.main.Value : clr->main.dark_4.Value, var->anim.speed, dynamic_easing);
    gui->easing(state->button_border, *value ? clr->accent.main.Value : clr->main.border.Value, var->anim.speed, dynamic_easing);
    gui->easing(state->circle_color, *value ? ImVec4(1.f, 1.f, 1.f, 1.f) : ImVec4(1.f, 1.f, 1.f, 0.f), var->anim.speed, dynamic_easing);
    ...
    gui->easing(state->circle_pos, *value ? 1.f : 0.f, var->anim.speed, dynamic_easing);
    ...
    const float box_rnd = SCALE(3.f);
    draw->rect_filled(drawlist, button_bb.Min, button_bb.Max, draw->get_clr(state->button_background), box_rnd, ImDrawFlags_RoundCornersAll);
    draw->rect(drawlist, button_bb.Min, button_bb.Max, draw->get_clr(state->button_border), box_rnd, ImDrawFlags_RoundCornersAll);

    if (state->circle_pos > 0.01f) {
        const ImVec2 check_size = gui->text_size(var->font.icon6, var->icons.check.data());
        const ImVec2 center = button_bb.GetCenter();
        const ImVec2 check_min(center.x - check_size.x * 0.5f, center.y - check_size.y * 0.5f);
        const ImVec2 check_max(check_min.x + check_size.x, check_min.y + check_size.y);
        draw->text_clipped(drawlist, var->font.icon6, check_min, check_max, draw->get_clr(state->circle_color), var->icons.check.data());
    }
    if (is_color)
